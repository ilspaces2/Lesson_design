 ...
 * <p>
 * 2) PhantomReference попадает в ReferenceQueue только после выполнения finalize(), что значит
 * мы еще имеем доступ к объекту некоторое время.
 * <p>
 ...
 * Создаем хранилище. В нем есть очередь, которая необходима для ссылок.  Но эта очередь read-only, поэтому создаем свой список и в него
 * помещаем наши фантомные ссылки. Когда вызывает метод для утилизации ресурсов мы проверяем если ссылка в очереди, т.е. помечен ли объект на удаление.
